...
function renderZSH(data, messages) {
...
function renderVSCodeLoc(data, messages) {
...
function renderBrewLoc(data, messages) {
    const { machineData: mD, installData: iD, installValidation: iV } = data;
...
function renderVSCodeAlias(data, messages) {
...
function renderGHExists(data, messages) {
    const { installData: iD, installValidation: iV } = data;
    if (iV.isValidGHLoc) {
        messages.successes.push({
...
function renderNVMExists(data, messages) {
...
function renderInstallLocAndVer(data, messages) {
    const { installValidation: iV } = data;
    const nodeVer = iV.versions.find(version => (version.name = 'nodeVer'));
    const isNodeVerValid = nodeVer?.isValid ? true : false;
...
function renderNodeStatus(data, messages, ver) {
...
function renderNodemonStatus(data, messages, ver) {
    const { installData: iD } = data;
    if (ver.isValid) {
        messages.successes.push({
...
function renderHerokuExists(data, messages) {
    const { installData: iD, installValidation: iV } = data;
